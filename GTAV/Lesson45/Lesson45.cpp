...
#include "TextureClass.h"
    ...
    geometry extract(char *arg)
    {
            ...
            try
            {
					...
					//Read textures
					in.seekg(0x94+0x10);
					global_textures = new classTextures(in);
                    in.seekg(0x50);
 ...
 vert28 *verts = (vert28*)global_geom.vbo.vbo;

 glBegin(GL_TRIANGLES);
  for(int i=0; i<global_geom.indexCount; i++ )
  {
   vert28 &vert = verts[global_geom.ibo.indicies[i]];
